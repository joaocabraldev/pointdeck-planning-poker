...
function getRoomResponse(
  room: PokerPlanningRoom,
  store: { getUser: (id: string) => { name: string } | undefined },
): RoomResponse {
...
router.delete(
  ...
  (req: AuthRequest, res: Response) => {
    ...
    if (room.owner !== userId) {
      return res
        .status(403)
        .json({ error: 'Only the room owner can remove participants' });
    }
...
router.post(
  ...
  (req: AuthRequest, res: Response) => {
    ...
    if (room.owner !== userId) {
      return res
        .status(403)
        .json({ error: 'Only the room owner can transfer ownership' });
    }
    ...
    if (!room.participants.includes(newOwnerId)) {
      return res
        .status(400)
        .json({ error: 'Target user is not a participant in this room' });
    }
...
const startVoting = (req: AuthRequest, res: Response) => {
  ...
  if (room.owner !== userId) {
    return res
      .status(403)
      .json({ error: 'Only the room owner can start voting' });
  }
...
const closeVoting = (req: AuthRequest, res: Response) => {
  ...
  if (room.owner !== userId) {
    return res
      .status(403)
      .json({ error: 'Only the room owner can close voting' });
  }
...
const resetVoting = (req: AuthRequest, res: Response) => {
  ...
  if (room.owner !== userId) {
    return res
      .status(403)
      .json({ error: 'Only the room owner can reset voting' });
  }
...
router.post(
  ...
  (req: AuthRequest, res: Response) => {
    ...
    if (room.owner !== userId) {
      return res
        .status(403)
        .json({ error: 'Only the room owner can set the agreed value' });
    }
    ...
    if (room.votingStatus !== 'closed') {
      return res
        .status(400)
        .json({ error: 'Voting must be closed before setting agreed value' });
    }
